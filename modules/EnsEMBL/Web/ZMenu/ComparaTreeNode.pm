...
##EG/ParaSite: allow re-alignment of tree
    ...
    $self->add_entry({
      type  => 'Export sub-tree',
      label => 'Independently align sequences in sub-tree (slow)',
      link  => $url_ondemand,
      extra => { external => 1 },
      order => 16
    });
...
## ParaSite: we can simplify our call to Wasabi
    my $gt_id = $node->tree->stable_id;
    my $rest_url = sprintf('%s/genetree/id/%s?content-type=text/javascript&aligned=1&subtree_node_id=%s',
