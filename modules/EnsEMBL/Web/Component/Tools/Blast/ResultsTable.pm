...
sub content {
  ...
  if ($job && $job->status eq 'done' && @{$job->result}) {
    ...
    my $table       = $self->new_table($source =~/latestgp/i
      ? [
## ParaSite
          { 'key' => 'species', 'title'=> 'Genome',           'align' => 'left',  'sort' => 'string'  },
##
...
##
      ...
      [], 
      {
        ...
        'sorting' => ['score desc'],
## EG & ParaSite          
        'hidden_columns' => $source =~/latestgp/i ? [3, 4, 5, 6] : [3, 4, 5, 6, 7, 8, 11] }
##    
    );
    ...
    for (@$results) {
      ...
      $result_data->{'species'} = $hub->species_defs->get_config($result_data->{'species'}, 'SPECIES_COMMON_NAME'); 
      ##
